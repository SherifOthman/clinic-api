...
public class ClinicBranchRepository : IClinicBranchRepository
{
    ...
    public async Task<ClinicBranch?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
    ...
    {
        const string sql = @"
            INSERT INTO ClinicBranches (ClinicId, Name, AddressLine, CountryGeoNameId, StateGeoNameId, CityGeoNameId, IsMainBranch, IsActive, CreatedAt, IsDeleted)
            OUTPUT INSERTED.Id
            VALUES (@ClinicId, @Name, @AddressLine, @CountryGeoNameId, @StateGeoNameId, @CityGeoNameId, @IsMainBranch, @IsActive, GETUTCDATE(), 0)";
