...
public class GeoLocationSeedService
{
    ...
    // Inline DbSets — same pattern as repositories
    ...
    {
        if (await _countries.AnyAsync(ct))
        {
        ...
        var existingIds = await _countries.Select(c => c.GeonameId).ToHashSetAsync(ct);
        ...
        if (newRows.Count > 0)
        {
            await _countries.AddRangeAsync(newRows, ct);
    ...
    private async Task SeedStatesAsync(CancellationToken ct)
    {
        ...
        var existingIds    = await _states.Select(s => s.GeonameId).ToHashSetAsync(ct);
        var validCountries = await _countries.Select(c => c.GeonameId).ToHashSetAsync(ct);
        ...
        if (newRows.Count > 0)
        {
            await _states.AddRangeAsync(newRows, ct);
            await _db.SaveChangesAsync(ct);
        }
    ...
    {
        if (await _cities.AnyAsync(ct))
        {
            _logger.LogInformation("Cities already seeded — skipping.");
            return;
        }

        var validStates = await _states.Select(s => s.GeonameId).ToHashSetAsync(ct);
        ...
        var existingIds = await _cities.Select(c => c.GeonameId).ToHashSetAsync(ct);
        ...
        await foreach (var c in _geoNames.StreamCitiesAsync(ct))
        {
            ...
            await _cities.AddRangeAsync(batch, ct);
            await _db.SaveChangesAsync(ct);
