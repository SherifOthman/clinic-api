...
public class AuthController : BaseApiController
{
    ...
    public async Task<IActionResult> GetMe(CancellationToken cancellationToken)
    {
        ...
        if (!result.Success)
            return Unauthorized(new ApiError(result.Message ?? ApplicationErrors.Authentication.AUTHENTICATION_FAILED));
    ...
    [Produces("application/json")]
    [ProducesResponseType(typeof(UpdateProfileImageResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status401Unauthorized)]
    public async Task<IActionResult> UpdateProfileImage([FromForm] IFormFile image, CancellationToken cancellationToken)
    {
        var command = new UpdateProfileImageCommand { Image = image };
        var result = await Mediator.Send(command, cancellationToken);
        return HandleResult(result);
    }
